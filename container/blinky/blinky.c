...
static QState Blinky_state1(Blinky * const me, QEvt const * const e);
//$enddecl${AOs::Blinky} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
static QState Blinky_initial(Blinky * const me, void const * const par) {
    ...
    QS_FUN_DICTIONARY(&Blinky_state1);
